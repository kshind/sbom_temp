...
def load_components_from_data(data: dict[str, Any]) -> list[Component]:
...
def resolve_sbom_source(source):
    ...
    if text.startswith("http://") or text.startswith("https://"):
        request = urllib.request.Request(
            text, headers={"User-Agent": "sbom-security-mcp/0.1"}
        )
        try:
            with urllib.request.urlopen(request, timeout=timeout) as response:
                data = json.loads(response.read().decode("utf-8"))
        except urllib.error.URLError as error:
            raise ValueError(f"Could not fetch SBOM from URL: {error}") from error
        except json.JSONDecodeError as error:
            raise ValueError(f"URL did not return valid JSON: {error}") from error
        label = text.rstrip("/").rsplit("/", 1)[-1] or text
        return data, label
    ...
    if text.startswith("{") or text.startswith("["):
        try:
            ...
            raise ValueError(f"Inline SBOM content is not valid JSON: {error}") from error
        return data, "inline-sbom"

    sbom_path = Path(source)
    if not sbom_path.exists():
        raise FileNotFoundError(
            f"No such file: {sbom_path}. Pass a local path, an https:// URL, "
            "or the raw SBOM JSON text itself."
        )
    data = json.loads(sbom_path.read_text(encoding="utf-8"))
    return data, sbom_path.stem
...
def analyze_sbom_file(
    ...
    data, inferred_label = resolve_sbom_source(path)
    components = load_components_from_data(data)
    ...
    candidate_label = label or inferred_label
    return AnalysisResult(
        label=candidate_label,
        path=str(path) if len(str(path)) < 200 else f"<inline:{inferred_label}>",
...
def _dedupe_labels(candidates: list[AnalysisResult]) -> None:
    """Disambiguate candidates that share a label (e.g. multiple inline-JSON sources)."""
    seen: dict[str, int] = {}
    ...
    for candidate in candidates:
        if seen[candidate.label] > 1:
            counters[candidate.label] = counters.get(candidate.label, 0) + 1
            candidate.label = f"{candidate.label} #{counters[candidate.label]}"
